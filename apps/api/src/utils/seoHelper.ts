 ...
 */
export function generateMetaTags(article: SEOArticleInput) {
 ...
 */
export function generateStructuredData(article: SEOArticleInput) {
  const baseUrl = process.env["PUBLIC_KB_BASE_URL"] || "https://yourdomain.com/kb/p";
  const canonicalUrl = article.canonical_url || `${baseUrl}/${article.slug}`;
  const authorName = article.author_name || article.author?.name || "System";
 ...
 */
export function generateSitemap(articles: SitemapArticleInput[]): string {
  const baseUrl = process.env["PUBLIC_KB_BASE_URL"] || "https://yourdomain.com/kb/p";
 ...
 */
export function sanitizeMetaContent(content: string, maxLength = 160): string {
