 ...
 */
export async function enqueueEvent(eventId: string, event: string, payload: any): Promise<boolean> {
 ...
 */
async function processEventItem(eventItem: any): Promise<void> {
  ...
  try {
    ...
    switch (event) {
      ...
      case "subscription.created":
        ...
        await getOrFetchCustomerCredits(crmCustomerId).catch(err =>
          console.error(`[CRM Queue] Error recalculating credits after subscription.created:`, err)
        );
      ...
      case "subscription.renewed":
        ...
        CrmCacheManager.invalidateSubscriptions(crmCustomerId);
        await getOrFetchCustomerCredits(crmCustomerId).catch(err =>
          console.error(`[CRM Queue] Error recalculating credits after subscription update:`, err)
        );
...
function notifyClient(crmCustomerId: string | null | undefined, eventName: string, payload: Record<string, unknown>): void {
...
async function handleCustomerCreatedEvent(crmCustomerId: string): Promise<void> {
  if (!crmCustomerId) throw new Error("Missing crmCustomerId for customer.created");
...
async function handleCustomerUpdatedEvent(crmCustomerId: string, data: any): Promise<void> {
  if (!crmCustomerId) throw new Error("Missing crmCustomerId for customer.updated");
...
async function handleCustomerStatusChange(crmCustomerId: string, status: "ACTIVE" | "DEACTIVATED"): Promise<void> {
  if (!crmCustomerId) throw new Error(`Missing crmCustomerId for customer status change to ${status}`);
...
async function handleDomainUpsert(crmCustomerId: string, data: any): Promise<void> {
  if (!crmCustomerId) throw new Error("Missing crmCustomerId for domain upsert");

  const crmDomainId = String(data.crmDomainId || data.domain_id || data.domainId || "");
  if (!crmDomainId) throw new Error("Missing crmDomainId for domain upsert");
...
async function handleDomainDeleted(crmCustomerId: string, data: any): Promise<void> {
  if (!crmCustomerId) throw new Error("Missing crmCustomerId for domain delete");

  const crmDomainId = String(data.crmDomainId || data.domain_id || data.domainId || "");
  if (!crmDomainId) throw new Error("Missing crmDomainId for domain delete");
...
async function handleSubscriptionUpsert(crmCustomerId: string, data: any): Promise<void> {
  if (!crmCustomerId) throw new Error("Missing crmCustomerId for subscription upsert");

  const crmSubscriptionId = String(data.crmSubscriptionId || data.sub_id || data.id || "");
  if (!crmSubscriptionId) throw new Error("Missing crmSubscriptionId for subscription upsert");
...
async function handleSubscriptionDeleted(crmCustomerId: string, data: any): Promise<void> {
  if (!crmCustomerId) throw new Error("Missing crmCustomerId for subscription delete");

  const crmSubscriptionId = String(data.crmSubscriptionId || data.sub_id || data.id || "");
  if (!crmSubscriptionId) throw new Error("Missing crmSubscriptionId for subscription delete");
...
async function handleServiceUpsert(data: any): Promise<void> {
  const crmServiceId = String(data.crmServiceId || data.service_id || data.id || "");
  if (!crmServiceId) throw new Error("Missing crmServiceId for service upsert");
...
async function handleServiceDeleted(data: any): Promise<void> {
  const crmServiceId = String(data.crmServiceId || data.service_id || data.id || "");
  if (!crmServiceId) throw new Error("Missing crmServiceId for service delete");
