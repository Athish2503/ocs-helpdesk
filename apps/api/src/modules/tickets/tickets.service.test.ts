...
describe("Tickets Service - Business Logic & Edge Cases", () => {
  ...
  describe("createTicket()", () => {
    it("should throw 400 if category is inactive or missing", async () => {
      (prisma.category.findUnique as any).mockResolvedValue({ id: "cat1", isActive: false });

      await expect(
        createTicket(
          { title: "Help me", description: "Details", priority: "MEDIUM", categoryId: "cat1" },
          "cust1",
          "CUSTOMER"
        )
    ...
    it("should fallback to 'Other Services' category if categoryId is omitted", async () => {
      (prisma.category.findFirst as any).mockResolvedValue({ id: "cat_default", name: "Other Services", isActive: true });
      (prisma.category.findUnique as any).mockResolvedValue({ id: "cat_default", name: "Other Services", isActive: true });
      (prisma.routingRule.findUnique as any).mockResolvedValue(null);
      (prisma.user.findUnique as any).mockResolvedValue({ crmCustomerId: "crm1", email: "cust@example.com" });
      (prisma.slaPolicy.findFirst as any).mockResolvedValue(null);
      ...
      (prisma.ticketStatusHistory.create as any).mockResolvedValue({});

      const ticket = await createTicket(
        { title: "Help me", description: "Details", priority: "MEDIUM" },
        "cust1",
        "CUSTOMER"
      );
    ...
    it("should append secondary email note when created by secondary email", async () => {
      (prisma.category.findUnique as any).mockResolvedValue({ id: "cat1", name: "Billing", isActive: true });
      (prisma.routingRule.findUnique as any).mockResolvedValue(null);
      (prisma.user.findUnique as any).mockResolvedValue({ id: "cust1", crmCustomerId: "crm1", email: "primary@example.com" });
      (prisma.crmCustomer.findUnique as any).mockResolvedValue({ secondaryEmail: "sec@example.com" });
      (prisma.slaPolicy.findFirst as any).mockResolvedValue(null);
      ...
      await createTicket(
        { title: "Billing issue", description: "Details", priority: "MEDIUM", categoryId: "cat1" },
        "cust1",
        "CUSTOMER",
        "sec@example.com"
      );
  ...
  describe("getTicketById() & ABAC Security", () => {
    it("should throw 404 if ticket does not exist", async () => {
      ...
      await expect(
        getTicketById("nonexistent", { id: "cust1", email: "c@e.com", role: "CUSTOMER" })
    ...
    it("should throw 403 if customer tries to access another customer's ticket", async () => {
      ...
      await expect(
        getTicketById("t1", { id: "attacker_id", email: "att@e.com", role: "CUSTOMER" })
    ...
    it("should throw 403 if agent tries to access a ticket assigned to a team they do not belong to", async () => {
      ...
      await expect(
        getTicketById("t1", { id: "agent_bravo", email: "a@e.com", role: "AGENT" })
      ).rejects.toMatchObject({ statusCode: 403, message: expect.stringContaining("do not belong to the team") });
    });
  });
  ...
  describe("updateTicket()", () => {
    it("should throw 403 if customer attempts to change priority or status", async () => {
      ...
      await expect(
        updateTicket(
          "t1",
          { priority: "URGENT" },
          { id: "cust1", email: "c@e.com", role: "CUSTOMER" }
        )
      ).rejects.toMatchObject({ statusCode: 403, message: expect.stringContaining("Customers cannot change ticket priority") });
    });
    ...
    it("should throw 409 on optimistic lock conflict if updatedAt is outdated", async () => {
      ...
      await expect(
        updateTicket(
          "t1",
          { status: "IN_PROGRESS", updatedAt: "2026-01-01T10:00:00Z" },
          { id: "agent1", email: "a@e.com", role: "AGENT" }
        )
    ...
    it("should calculate credit consumption when resolving ticket with outside OCS domain rate", async () => {
      ...
      (prisma.crmDomain.findFirst as any).mockResolvedValue(null); // Not in OCS
      (prisma.user.findUnique as any).mockResolvedValue({ crmCustomerId: "crm1" });
      (prisma.ticketStatusHistory.create as any).mockResolvedValue({});
      (prisma.creditUsage.create as any).mockResolvedValue({});
      ...
      const updated = await updateTicket(
        "t1",
        { status: "RESOLVED", hoursConsumed: 1 },
        { id: "agent1", email: "a@e.com", role: "AGENT" }
      );
