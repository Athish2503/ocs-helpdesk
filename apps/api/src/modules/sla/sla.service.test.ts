...
describe("SLA Service - Business Logic & Edge Cases", () => {
  ...
  it("listSlaPolicies should return policies ordered by priority tier", async () => {
    (prisma.slaPolicy.findMany as any).mockResolvedValue([
  ...
  it("createSlaPolicy should throw 409 if policy already exists for priority tier", async () => {
    (prisma.slaPolicy.findFirst as any).mockResolvedValue({ id: "existing", priority: "HIGH" });

    await expect(
      createSlaPolicy({ name: "High Policy", priority: "HIGH", firstResponseHours: 2, resolutionHours: 8, isActive: true })
  ...
  it("computeSlaDeadlines should return correct deadlines when policy exists", async () => {
    (prisma.slaPolicy.findFirst as any).mockResolvedValue({
      id: "p1",
      priority: "URGENT",
