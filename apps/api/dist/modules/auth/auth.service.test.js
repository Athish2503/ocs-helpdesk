...
import { register, login, refresh, getMe, forgotPassword, resetPassword, setupPassword, verifyInvitationToken, } from "./auth.service.js";
describe("Auth Module - Business Logic & Edge Cases", () => {
    ...
    describe("register()", () => {
        it("should throw 400 because self-registration is disabled", async () => {
            await expect(register({ name: "John", email: "john@example.com", password: "pwd" })).rejects.toThrow("Self-registration is disabled");
        });
    });
    describe("login()", () => {
