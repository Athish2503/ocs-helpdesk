...
describe("CRM Queue Service - Idempotency & Ingestion", () => {
    ...
    it("enqueueEvent should ignore duplicate events with identical eventId", async () => {
        prisma.crmSyncLog.findUnique.mockResolvedValue({ id: "log1", eventId: "evt_dup_1" });
    ...
    it("enqueueEvent should insert new event into queue and log receipt", async () => {
        prisma.crmSyncLog.findUnique.mockResolvedValue(null);
        prisma.crmEventQueue.create.mockResolvedValue({ id: "q1" });
        prisma.crmSyncLog.create.mockResolvedValue({ id: "log1" });
        prisma.crmEventQueue.findMany.mockResolvedValue([]);
        const result = await enqueueEvent("evt_new_1", "customer.created", { crmCustomerId: "cust_1" });
        expect(result).toBe(true);
        expect(prisma.crmEventQueue.create).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                eventId: "evt_new_1",
                status: "PENDING",
            }),
        }));
        expect(prisma.crmSyncLog.create).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                eventId: "evt_new_1",
                status: "QUEUED",
            }),
