        ...
        <section className="max-w-7xl mx-auto px-6 pt-16 pb-20 md:py-24 grid lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-6">
            <div className={`w-full rounded-2xl overflow-hidden transition-all duration-300 ${
              isDark 
                ? 'bg-[#0A0F1E]/70 border border-[#1E293B] shadow-[0_20px_50px_rgba(0,0,0,0.5)]' 
                : 'bg-white border border-[#E2E8F0] shadow-[0_20px_50px_rgba(148,163,184,0.15)]'
            }`}>
              {/* Mockup Header */}
              <div className={`h-10 px-4 flex items-center justify-between border-b transition-colors ${
                ...
                <div className="text-[10px] font-mono text-[#64748B] select-none">
                  helpdesk.ocs365.in/dashboard
                </div>
                <div className="w-12 h-2"></div> {/* Spacer to align title */}
              </div>

              {/* Mockup Content */}
              <div className={`p-6 transition-colors font-sans space-y-6 ${isDark ? 'bg-[#030712]/50' : 'bg-white'}`}>
                {/* Mockup Top Info */}
                <div className={`flex items-center justify-between border-b pb-4 transition-colors ${
                  isDark ? 'border-[#1E293B]/60' : 'border-[#E2E8F0]'
                }`}>
                  <div>
                    <h4 className={`text-xs font-bold transition-colors ${isDark ? 'text-white' : 'text-[#0F172A]'}`}>Support Tickets</h4>
                    <p className="text-[9px] text-[#64748B]">Review status and request history</p>
                  </div>
                  <div className={`flex rounded-lg p-1 space-x-1 text-[9px] font-semibold font-mono transition-colors ${
                    isDark ? 'bg-[#0F172A] border border-[#1E293B]' : 'bg-[#F1F5F9] border border-[#E2E8F0]'
                  }`}>
                    <button 
                      onClick={() => setActiveMockupTab('all')}
                      className={`px-2 py-0.5 rounded transition-all ${
                        activeMockupTab === 'all' 
                          ? 'bg-[#129FF0] text-[#020617] font-bold' 
                ...
                <div className="space-y-3">
                  {filteredTickets.map((ticket, i) => (
                    <div key={i} className={`p-3 border rounded-lg flex items-center justify-between transition-colors ${
                      isDark ? 'bg-[#0F172A]/80 border-[#1E293B] hover:border-[#38B1F7]/30' : 'bg-[#F8FAFC] border-[#E2E8F0] hover:border-[#129FF0]/30'
                    }`}>
                      <div className="space-y-1 overflow-hidden pr-3 text-left">
                        ...
                        <h5 className={`text-[11px] font-semibold truncate transition-colors ${isDark ? 'text-[#F8FAFC]' : 'text-[#0F172A]'}`} title={ticket.subject}>
                          {ticket.subject}
                        </h5>
                      </div>
                      <div className="flex items-center space-x-3 shrink-0">
                        ...
                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold tracking-wider uppercase font-mono ${
                          ticket.status === 'Open' 
                            ? isDark ? 'bg-blue-950/50 text-[#38B1F7] border border-[#38B1F7]/25' : 'bg-blue-50 text-[#129FF0] border border-[#129FF0]/20'
                            : isDark ? 'bg-green-950/50 text-[#12B76A] border border-[#12B76A]/25' : 'bg-green-50 text-[#12B76A] border border-[#12B76A]/20'
                        }`}>
                          {ticket.status}
                        </span>
                      </div>
